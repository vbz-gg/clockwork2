...
import { scanImportGraph } from "../../src/validate/scan/import-graph"
...
describe("following imports", () => {
  ...
  test("a trusted package is recorded as trusted and not walked", () => {
    const { result } = scan({
      "index.ts":
        'import { Prng } from "@clockwork2/engine"\nexport default Prng\n',
    })
    expect(result.trusted).toEqual(["@clockwork2/engine"])
    expect(result.files.every((f) => !f.includes("packages/engine"))).toBe(true)
  })
